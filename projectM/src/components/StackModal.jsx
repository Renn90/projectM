...
import UserContext, { Context } from "../pages/Auth/UserContext";
...
const StackModal = ({ addStack }) => {
  ...
  const addStackHandler =(e)=> {
   setAddStackTemp(()=>[...addStackTemp,e]) 
  }
      ...
      const response = await fetch(sanityAPI,
        {
          ...
          body: JSON.stringify({
            mutations: [{ patch:  { _type: 'user', id: userid, set: { stack: stackData } } }],
          }),
        }
       )
        ...
        <div>
         ...
         { addStackTemp.length > 0 && <div className="flex items-center">
          <div className="flex flex-wrap rounded-lg w-full max-h-[80px] overflow-hidden overflow-y-scroll bg-grey slidebar p-2">
            ...
            <p className="text-xs bg-white p-2 m-1 rounded relative" key={index}>{tool.name}
            <BiX className="absolute top-[-6px] text-white right-[-8px] text-[15px] bg-[grey] rounded-full cursor-pointer"
            onClick={()=> removeTempHandler(tool.name)}
            />
            </p>
            ))}
            </div>
            <button className="bg-secondary p-2 text-white ml-2 text-xl h-[40px] w-[40px] font-semibold rounded hover:opacity-70" onClick={saveStack}>+</button>
          </div>}
          <select className="w-full  bg-grey my-2 outline-none p-2 rounded text-black" value={cartegory} onChange={pickStack}>
